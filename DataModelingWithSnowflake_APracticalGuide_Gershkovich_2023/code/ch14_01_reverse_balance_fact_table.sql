...
-- Create initial load of the reverse balance fact table (No changes yet)
CREATE OR REPLACE TABLE line_item_rb (
    __load_type varchar, -- Will make it easier to understand the records in this example
    as_at_dts timestamp_ntz NOT NULL, -- In a daily load, this should be a DATE field
    is_after_image boolean NOT NULL,
    ...
    comment varchar(44),
    -- This example does NOT use a diff_hash because matching incoming records are understood to be updates
    --  - Otherwise, use a diff_hash for easy compare
    -- diff_hash varchar(32)

    CONSTRAINT pk_line_item PRIMARY KEY (line_number, sales_order_id, as_at_dts, is_after_image) -- NOTE: No RELY property = PK constraint NOT enforced
)
...
CREATE OR REPLACE TABLE line_item_rb_bak CLONE line_item_rb;
...
-- Load source table with 10 new and 10 updated (whole) orders
-- CTE 1
WITH new_not_existing AS (
    SELECT
        DISTINCT sales_order_id
    FROM source_system_line_item
    SAMPLE (10 ROWS)
    EXCEPT -- Set operator to removes rows from 1st query’s result set which appear in 2nd query’s result set, with duplicate elimination
    SELECT
        DISTINCT sales_order_id
    FROM src_line_item
),
...
INNER JOIN new_not_existing AS new
    ON src.sales_order_id = new.sales_order_id
...
SELECT * FROM line_item_rb WHERE __load_type = 'deletion';
SELECT * FROM line_item_rb WHERE sales_order_id = '1284414' ORDER BY line_number;
